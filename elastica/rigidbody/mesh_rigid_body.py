...
class MeshRigidBody(RigidBodyBase):
    def __init__(
        ...
        self.tangents = self.director_collection[2, ...]

        mesh_scale = getattr(mesh, "mesh_scale", None)
        if mesh_scale is None:
            mesh_scale = np.zeros((3,))
        self.length = np.float64(np.max(mesh_scale))
        self.radius = np.float64(0.5 * np.max(mesh_scale))
        ...
        self.external_forces = np.zeros((MaxDimension.value(), 1))
        self.external_torques = np.zeros((MaxDimension.value(), 1))
        self.internal_forces = np.zeros((MaxDimension.value(), 1))
        self.internal_torques = np.zeros((MaxDimension.value(), 1))
