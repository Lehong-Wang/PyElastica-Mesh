...
def run_multi_rod_drop(
    ...
    direction = np.array([1.0, 0.0, 0.2]) / np.sqrt(1.04)
    normal = np.array([-0.2, 0.0, 1.0]) / np.sqrt(1.04)
    ...
    for i in range(num_rods):
        ...
        jitter_pos = (rng.random(3) - 0.5) * 0.06  # ±0.02 m
